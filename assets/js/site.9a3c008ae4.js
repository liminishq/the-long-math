...
(function () {
  ...
  // -------------------------
  // Calculator numeric inputs (decimal keypad + 2-decimal entry)
  // -------------------------
  function loadCalcInputsHelper() {
    var path = (location.pathname || "");
    if (path.indexOf("/calculators/") === -1) return;
    if (window.TLM && window.TLM.calcInputs) {
      window.TLM.calcInputs.initCalculatorInputs();
      return;
    }
    if (document.querySelector('script[data-tlm-calc-inputs]')) return;
    var s = document.createElement("script");
    s.src = "/assets/js/calc-inputs.js";
    s.async = true;
    s.setAttribute("data-tlm-calc-inputs", "1");
    document.head.appendChild(s);
  }
