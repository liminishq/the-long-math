...
(function () {
  ...
  function collectKeyValueFromScope(scope, rows, seen) {
    ...
    [
      ["horizonTfsa", "TFSA at horizon (after-tax)"],
      ["horizonRrsp", "RRSP at horizon (after-tax)"],
      ["horizonFhsa", "FHSA at horizon (after-tax)"],
      ["horizonNonReg", "Non-registered at horizon (after-tax, no drag)"]
    ].forEach(function (pair) {
      var el = scope.querySelector("#" + pair[0]);
      if (!el || !isVisible(el)) return;
      var cell = el.closest(".pri");
      if (cell && !isVisible(cell)) return;
      var text = normText(el.textContent);
      if (!text || isPlaceholder(text)) return;
      addPair(rows, seen, pair[1], text);
    });
